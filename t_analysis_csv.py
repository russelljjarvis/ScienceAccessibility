...
#data = zlib.compress(base64.b64decode())


def web_iter(frames = False):
    import pandas as pd
    visited_files = []
    ...
    for p,fileName in enumerate(lo_query_links):
        ...
        for index in range(0,len(file_contents)):
            ...
            if file_contents.iloc[index]['status']=='successful' and len(file_contents.iloc[index]['snippet']) > 100 and detectedLangName == 'ENGLISH':
                ...
                #Data frames are best but computationally expensive to generate
                if frames == True:
                    if textstat.flesch_kincaid_grade(str(tokens)) > 100.0 or testimonial.sentiment.subjectivity == 0 or urlDat['eofh'] == 0:
                        pass
                    else:
